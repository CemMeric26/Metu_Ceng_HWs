...
std::string operator+(const HighSpeedTrain &h1, const HighSpeedTrain &h2){
    ...
    //output=std::cout << h1.from << " -> " << h1.to << " -> "<< h2.to << " in "<< h1.eta+h2.eta  <<" mins.";
    out= std::to_string(h1.eta+h2.eta);
   
    return h1.from + " -> " + h1.to + " -> " + h2.to + " in " + out  + " mins.";  ; // ADJUST THIS AFTER YOUR IMPLEMENTATION IS DONE.
}
...
bool operator<(const HighSpeedTrain &h1, const HighSpeedTrain &h2){
    ...
    else if(h1.eta > h2.eta)
    {
        return false;
    }
    
    return true; // ADJUST THIS AFTER YOUR IMPLEMENTATION IS DONE.
}
